...
def save_annotations(filename, objs, filepath, xml_path):
    annopath = xml_path + "/" + filename[:-3] + "xml"  # 生成的xml文件保存路径
    dst_path = xml_path + "/" + filename
    img_path = filepath
...
def showbycv(coco, xml_path, img, classes, origin_image_dir, verbose=False):
    ...
    save_annotations(filename, objs, filepath, xml_path)
    if verbose:
        cv2.imshow("img", I)
        cv2.waitKey(0)
...
def get_CK5(annpath, xml_path, origin_image_dir, verbose=False):
    ...
    for imgId in tqdm(imgIds):
        ...
        showbycv(coco, xml_path, img, classes, origin_image_dir, verbose=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--json_path', default=r'./json/output.json', type=str,    # json文件路径
                        help="input: coco format(json)")
    parser.add_argument('--xml_path', default=r'./xml', type=str,
                        help="specify where to save the output dir of labels")      # xml文件所在文件夹路径
    parser.add_argument('--img_path', default=r'./pic', type=str,
                        help="specify where to save the output dir of labels")      # img文件所在文件夹路径

    arg = parser.parse_args()

    annpath = arg.json_path
    xml_path = arg.xml_path
    img_path = arg.img_path

    get_CK5(annpath, xml_path, img_path, verbose=False)
